#default files
PRODUCT_COPY_FILES += \
	vendor/htc/passion/prebuilt/app/Cyanbread.apk:./system/app/Cyanbread.apk \
	vendor/htc/passion/prebuilt/app/LiveWallpapers.apk:./system/app/LiveWallpapers.apk \
	vendor/htc/passion/prebuilt/app/Music.apk:./system/app/Music.apk \
        vendor/htc/passion/prebuilt/app/ROMUpdater.apk:./system/app/ROMUpdater.apk \
	vendor/htc/passion/prebuilt/bin/sysinit:./system/bin/sysinit \
   	vendor/htc/passion/prebuilt/etc/init.d/40a2sd:./system/etc/init.d/40a2sd \
   	vendor/htc/passion/prebuilt/etc/init.d/99complete:./system/etc/init.d/99complete \
    	vendor/htc/passion/prebuilt/etc/apns-conf.xml:./system/etc/apns-conf.xml \
	vendor/htc/passion/prebuilt/etc/updatecmds/google_generic_update.txt:./system/etc/updatecmds/google_generic_update.txt  \
	vendor/htc/passion/prebuilt/lib/libnative.so:./system/lib/libnative.so \
        vendor/htc/passion/prebuilt/media/bootanimation.zip:./system/media/bootanimation.zip \
        vendor/htc/passion/prebuilt/media/audio/ui/Lock.ogg:./system/media/audio/ui/Lock.ogg \
        vendor/htc/passion/prebuilt/media/audio/ui/Unlock.ogg:./system/media/audio/ui/Unlock.ogg \
        vendor/htc/passion/prebuilt/sd/placeholder:./system/sd/placeholder

